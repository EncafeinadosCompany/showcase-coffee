...
import {fetchProductTop,fetchEarlyDate,fetchEarnings,fetchTotalLiquidation,fetchTotalDeposits} from "../../features/dashboard/dashboardSlice";


const Dashboard = () => {

  const dispatch = useDispatch<AppDispatch>();
  const { totalLiquidation, totalDeposits, isLoading, error } = useAppSelector(
  ...
  const NoDataComponent = () => (
    <div className="flex flex-col items-center justify-center h-96 rounded-2xl bg-gray-50 border-2 border-dashed border-gray-200">
      ...
      <p className="text-gray-500 mt-2 text-center max-w-md">
        No se encontraron registros de transacciones en el sistema. Por favor,
        agrega algunas transacciones para visualizar tus datos.
      </p>
    </div>
  );
  ...
  const MainMetricCard = ({
    ...
    return (
      <div
        className={`p-6 rounded-2xl overflow-hidden relative transition-all duration-700 transform ${
                ...
                <div className="p-4 bg-gray-50 rounded-xl">
                  ...
                  <div className="mt-4 flex items-center">
                    ...
                    >
                      {data.pagos - data.deudas >= 0 ? "+" : ""}
                      {Math.round(
                        ((data.pagos - data.deudas) / data.deudas) * 100
                      )}
                      %
                    </div>
                  </div>
