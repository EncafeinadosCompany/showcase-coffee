...
const TopProductsChart = ({
  ...
  const processedData = topProducts
    ? topProducts
        .map((product) => ({
          name: `${
            product.variantProduct.product.name.length > 20
              ? product.variantProduct.product.name.substring(0, 20) + "..."
              : product.variantProduct.product.name
          } (${product.variantProduct.grammage})`,
          ...
          <div className="grid grid-cols-2 gap-2 pt-2 border-t border-gray-100">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-lg font-bold text-gray-800">
                #{processedData.findIndex((item) => item.id === product.id) + 1}
              </span>
            </div>
          </div>
        </div>
    ...
    >
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart
              ...
              onClick={(data, index) =>
                handleBarClick(
                  data,
                  index.dataKey === "value" ? index.index : -1
                )
              }
            >
              <defs>
              ...
              >
                {chartData.map((_, index) => (
                  <Cell
                    key={`cell-${index}`}
                    fill={
                      activeIndex === index ? "#7C3AED" : "url(#barGradient)"
                    }
                    cursor="pointer"
      ...
      {chartData.length > 0 && (
        <div className="mt-4 flex items-start space-x-2 text-xs text-gray-500">
          ...
          <p>
            Mostrando top {displayCount} productos. Haz clic en las barras para
