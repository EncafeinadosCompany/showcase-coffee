...
export const fetchSocialNetworks = createAsyncThunk("socialNetworks/fetchAll", async (_, { rejectWithValue }) => {
    try {
        return await getSocialNetworks();
    } catch (error: any) {
        return rejectWithValue(error.response?.data?.message || "Error al obtener redes sociales");
    }
});

export const addSocialBrand = createAsyncThunk("socialNetworks/add", async (socialNetwork: Omit<SocialBrandType, "id">, { rejectWithValue }) => {
    try {
        return await createSocialBrand(socialNetwork);
    } catch (error: any) {
        return rejectWithValue(error.response?.data?.errors || "Error al agregar red social");
    }
});
